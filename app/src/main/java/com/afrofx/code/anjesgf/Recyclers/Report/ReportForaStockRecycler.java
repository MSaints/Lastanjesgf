...
public class ReportForaStockRecycler extends RecyclerView.Adapter<ReportForaStockRecycler.viewHolder> {

    private List<StockModel> stockModelList;
    private Context context;
    public ReportForaStockRecycler(Context context, List<StockModel> stockModelList){
    ...
    public void onBindViewHolder(viewHolder holder, int position) {
        ...
        double valor = stockModel.getProduto_preco_venda();
        String nome = stockModel.getProduto_nome();

        String valor2 = format("%,.2f", valor);

        if(stockModel.getProduto_quantidade()==0){
